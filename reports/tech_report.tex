...
The node is shared, and that is part of the measurement conditions rather than an aside:
during these runs four other training jobs held between 0.5 and 47\,GB on the remaining
cards, so the GPU was never exclusively ours. The single-GPU settings sweep (30 steps plus
five discarded warmups, coefficient of variation 0.1--0.6\%) is the least exposed to that
contention; the throughput columns of the FID tables are global rates from 4-GPU sharded
...
The spread column is worth reading carefully. A 10k subset reads about 1 FID unit *higher*
than the 50k measurement of the same sampler ($12.41 \pm 0.08$ vs.\ $11.45$ for DDIM at 50
NFE), which means the 10k ancestral number in the last row is if anything pessimistic: the
two rows' gap on equal footing is smaller than the table's raw numbers suggest. This is the
sample-count dependence the frozen protocol's variability requirement exists to expose, and
it is why the deterministic rows here are reported at 50k.
...
Two caveats belong next to that table, and they cut in opposite directions.
...
crossover sits near NFE 16, so the flow objective buys a $8\times$ reduction in evaluations
on the cheap end of the curve, and a $4.9\times$ lower FID on the expensive end. This is the
expected result --- a straighter path is easier to integrate --- and it reproduces on both
backbones, which is what makes it credible.
...
\textbf{\dots and it is not free.} Acting on the previous paragraph, one MNIST cell was
retrained under the identical protocol with the training step autocast to bf16 --- the point
being to price the speedup instead of assuming it. The training loss is unchanged to three
digits ($0.0194$ vs.\ $0.0193$), but the sampled quality is not: the bf16 checkpoint scores
FID $5.56$ where the fp32 one scores $2.79$ on the 1000-step ancestral sampler, while the two
sit within noise of each other at 50 steps ($4.10$ vs.\ $4.32$). That is the same signature
Section~\ref{sec:samplers} found for the transformer --- an error the long trajectory
accumulates and the short one hides --- appearing here from precision rather than from
training. One cell and one seed is thin evidence, and we report it as an observation, not a
law; but it is enough that we do not quote $3.15\times$ as free, and it is why no quality
number in this report comes from a mixed-precision run.

\textbf{Compilation buys nothing here.} \texttt{torch.compile} returns $1.00\times$ --- the same
step time to three digits --- which is a negative result worth stating plainly. Two
explanations are consistent with the profile: 67\% of GPU time is already inside cuBLAS
matmuls that have little to fuse, and the fusible elementwise work sits between large kernels
rather than accumulating in a long chain.

\textbf{Why FSDP loses at this scale.} FSDP is $12.8\%$ slower than DDP and saves only 4.8\%
of peak memory --- which is the prediction this benchmark was built to check. The peak is
dominated by activations: at batch 160 the model's parameters, gradients and Adam state
together account for roughly 2\,GB of the 33.5\,GB peak, so sharding them cannot help, while
the all-gather and reduce-scatter on the critical path cost real time. Fully sharded training
is a tool for models whose \emph{states} dominate their memory; at 130M parameters on 48\,GB
cards, that condition is not met.

\textbf{Reading the MFU column.} The $10.4\%$ fp32 and $32.7\%$ bf16 figures come from
measured FLOPs (46.0 GFLOPs per sample, forward, counted by
\texttt{torch.utils.flop\_counter}) against the A6000's dense peak. Two caveats apply: the
counter covers matmul and convolution work only, so elementwise-heavy operations are invisible
to it (a fused-adaLN kernel would have to earn its keep out of the 15.3\% slice in
Fig.~\ref{fig:profile}, not out of the FLOPs number); and this is a 130M-parameter model at
256 tokens, so neither the batch nor the sequence length is large enough to saturate tensor
cores. The honest summary is that bf16 moved this workload from "not using the tensor cores"
to "using them moderately".
...
\textbf{Learning rate $5\times10^{-4}$ collapses training.}
\emph{Hypothesis:} the 5$\times10^{-4}$ rate used by the first CelebA run was safe because
Adam is self-normalizing. \emph{Observation:} the loss diverged and the samples degraded into
noise; the run produced no usable checkpoint. \emph{Diagnosis:} at that rate the early
updates in the high-noise regime were large enough to leave the region where the
$\epsilon$-prediction loss is informative, and the optimizer could not return. \emph{Resolution:}
the protocol in this report uses $2\times10^{-4}$ with gradient clipping at 1.0, and logs the
gradient norm every step so the failure is visible before the loss is. \emph{Lesson:} the
learning rate and the clipping threshold are part of the protocol, not tuning details ---
which is why every cell here shares one pair of them.

\textbf{DataParallel deadlocks with a multi-worker DataLoader.}
\emph{Hypothesis:} \texttt{nn.DataParallel} is the cheap way to use several GPUs for a model
that fits on one. \emph{Observation:} the first multi-GPU configuration hung, reliably, with
no error output, when the DataLoader used worker processes. \emph{Diagnosis:} DataParallel
replicates the module per forward pass and serialises it through a single process, which
interacts badly with forked workers pinning memory. \emph{Resolution:} the training stack was
rewritten around torchrun-native DDP, which is what every run in this report uses.
\emph{Lesson:} for anything beyond a single forward pass, DDP is both simpler and faster.

\textbf{P2P hangs on this node's NUMA layout.}
\emph{Hypothesis:} with DDP in place, the remaining hang was a software bug.
\emph{Observation:} \texttt{nvidia-smi topo -m} showed the six GPUs split across two NUMA
domains (0--3 and 4--5) with the cross-domain link traversing PCIe \emph{and} the inter-socket
interconnect; a job spanning both domains hung in the first collective while a job confined to
one domain ran. \emph{Diagnosis:} NCCL's peer-to-peer transport over that link does not
complete. \emph{Resolution:} \texttt{NCCL\_P2P\_DISABLE=1}, plus a rule that DDP jobs stay
inside one domain --- both of which every multi-GPU measurement in this report honours.
\emph{Lesson:} on shared hardware the topology is part of the experiment setup, and it is
cheaper to measure it once than to debug it later.

\textbf{FSDP is a net loss at this scale.}
\emph{Hypothesis:} sharding parameters, gradients and optimizer state should reduce the
memory peak and let the batch grow. \emph{Observation:} at 130M parameters and batch 160 per
GPU, FSDP ran $12.8\%$ slower than DDP and saved 4.8\% of peak memory
(Table~\ref{tab:cost}). \emph{Diagnosis:} the peak is activation-dominated --- the sharded
states are $\sim$2\,GB of a 33.5\,GB peak --- so the all-gather pays for itself only when
parameters rather than activations set the ceiling. \emph{Resolution:} reported as a
deliberate reverse benchmark rather than removed; the criterion it establishes
(states $>$ activations) is the one to check before reaching for FSDP at this scale.
\emph{Lesson:} a negative result with a stated criterion is more useful than an unmeasured
technique in the toolbelt.
...
\begin{itemize}
  ...
  \item The reported timings use fp32 training, and no quality number in this report comes
        ...
        1000-step FID doubled. That is one seed and one cell --- an observation, not a
        characterisation of where the boundary lies.
  \item The transformer's deterministic-sampler collapse (Section~\ref{sec:samplers}) is
        characterised at one scale with one model. The $\eta$ sweep localises the mechanism
        --- the absence of noise injection --- but where the boundary lies, and whether it
        moves with more training or more parameters, is not measured.
\end{itemize}
