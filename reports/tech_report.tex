...
\begin{figure}[t]
  ...
  per-kernel accounting, which is precisely what a FLOPs count hides: the 15.6\% spent on
...
to it (a fused-adaLN kernel would have to earn its keep out of the 15.6\% slice in
...
\begin{itemize}
  ...
  \item The reported timings use fp32 training. bf16 was measured for \emph{speed}
        (Table~\ref{tab:cost}) but its effect on sample quality was not measured, so no
        quality number in this report comes from a mixed-precision run.
